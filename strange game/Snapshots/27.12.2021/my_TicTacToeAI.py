...
def insertLetter(letter, position):
    if isSpaceFree(position):
        ...
        if isDraw() or isWin():
            ...
            #This is only needed if we don't use 
            #the for-loop in the server script
            """#Updating remaining games to play:
            games_to_play = getGamesToPlay()
            games_to_play -= 1
            with open("games_to_play.txt","w") as f:
                f.write(str(games_to_play))""" 
...
def myMove(server_move):
    """ with open("server_move.txt","r") as f: 
        position = int(f.read())
    print("Server's move:",position) """
...
def botMove():
    #Bot is the minimizing play, so the lower
...
def getGamesToPlay():
    with open("games_to_play.txt","r") as f:
        games_to_play = int(f.read())
    return games_to_play

def resetMoves():
    with open("server_move.txt","w") as f:
        f.write("")
    with open("Bot_move.txt","w") as f:
        f.write("")
...
if __name__ == "__main__":
    #Outputting number of games to a file the server will access
    games_to_play  = input("Enter amount of games to play:")
    with open("games_to_play.txt","w") as f:
        f.write(games_to_play)
    
    #Time for me to enter games_to_play into console
    time.sleep(1.2) 
    
    server_regex.serverMain() 
